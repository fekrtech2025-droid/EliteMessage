...
export type AppShellSurface = 'neutral' | 'customer' | 'admin';
export type AppShellDensity = 'comfortable' | 'compact';
export type AppShellContentWidth = 'narrow' | 'normal' | 'wide' | 'full';
export type AppShellHeaderMode = 'default' | 'hidden';
...
type AppShellProps = PropsWithChildren<{
  ...
  headerMode?: AppShellHeaderMode;
  topbarBrandMarkSrc?: string;
  topbarBrandMarkAlt?: string;
  labels?: AppShellLabels;
}>;

type SurfaceBrandVariant = 'topbar' | 'rail';

type SurfaceBrandProps = {
  label: ReactNode;
  variant?: SurfaceBrandVariant;
  markSrc?: string;
  markAlt?: string;
  launcherControlsId?: string;
  launcherButtonLabel?: string;
  launcherExpandLabel?: string;
  onLauncherClick?: (event: MouseEvent<HTMLButtonElement>) => void;
};

const contentWidths: Record<Exclude<AppShellContentWidth, 'full'>, number> = {
  narrow: 760,
  normal: 1160,
  wide: 1360,
};

export function setShellNavCollapsed(
  shell: HTMLElement,
  collapsed: boolean,
  controlsId?: string,
) {
  if (collapsed) {
    ...
    shell.removeAttribute('data-nav-collapsed');
  }

  const frame = shell.querySelector<HTMLElement>('.elite-shell-frame');
  if (frame) {
    if (collapsed) {
      frame.setAttribute('data-nav-collapsed', 'true');
    } else {
      frame.removeAttribute('data-nav-collapsed');
    }
  }

  const aside = controlsId
    ? shell.querySelector<HTMLElement>(`#${controlsId}`)
    : shell.querySelector<HTMLElement>('.elite-shell-aside');
  if (aside) {
    aside.hidden = false;
    aside.removeAttribute('aria-hidden');
  }

  const mobileNav = shell.querySelector<HTMLElement>('.elite-shell-mobile-nav');
  if (mobileNav) {
    mobileNav.hidden = collapsed;
  }

  const launcher = shell.querySelector<HTMLButtonElement>(
    '.elite-shell-topbar-brand-launcher',
  );
  if (launcher) {
    const collapseLabel =
      launcher.dataset.collapseLabel ?? 'Collapse sidebar menu';
    const expandLabel = launcher.dataset.expandLabel ?? 'Expand sidebar menu';
    launcher.dataset.state = collapsed ? 'collapsed' : 'expanded';
...
function surfaceLabel(surface: AppShellSurface) {
  switch (surface) {
    case 'customer':
...
export function SurfaceBrand({
  ...
  const launcherIcon = (
    <svg viewBox="0 0 24 24" fill="none" aria-hidden="true">
      <rect x="4" y="4" width="4.5" height="4.5" rx="1.1" />
      <rect x="9.75" y="4" width="4.5" height="4.5" rx="1.1" />
      <rect x="15.5" y="4" width="4.5" height="4.5" rx="1.1" />
      <rect x="4" y="9.75" width="4.5" height="4.5" rx="1.1" />
      <rect x="9.75" y="9.75" width="4.5" height="4.5" rx="1.1" />
      <rect x="15.5" y="9.75" width="4.5" height="4.5" rx="1.1" />
      <rect x="4" y="15.5" width="4.5" height="4.5" rx="1.1" />
      <rect x="9.75" y="15.5" width="4.5" height="4.5" rx="1.1" />
      <rect x="15.5" y="15.5" width="4.5" height="4.5" rx="1.1" />
    </svg>
  );
  const launcher = isTopbar ? (
    onLauncherClick ? (
      <button
        type="button"
        ...
        data-state="expanded"
        data-collapse-label={launcherButtonLabel}
        data-expand-label={launcherExpandLabel}
        aria-label={launcherButtonLabel}
        aria-controls={launcherControlsId}
        aria-expanded="true"
      ...
      <span className="elite-shell-topbar-brand-launcher" aria-hidden="true">
        {launcherIcon}
      </span>
    )
  ) : null;
  ...
  return (
    <div
      className={isTopbar ? 'elite-shell-topbar-brand' : 'elite-rail-brand'}
      data-variant={variant}
    >
      <span
        className={
          isTopbar ? 'elite-shell-topbar-brand-mark' : 'elite-rail-brand-mark'
        }
        aria-hidden="true"
        data-has-image={markSrc ? 'true' : 'false'}
      >
        {markSrc ? (
          <img
      ...
      <div
        className={
          isTopbar ? 'elite-shell-topbar-brand-copy' : 'elite-rail-brand-copy'
        }
      >
        <span
          className={
            isTopbar ? 'elite-shell-topbar-brand-name' : 'elite-rail-brand-name'
          }
        >
          Elite Message
        </span>
...
export function AppShell({
  ...
  const navRegionId = nav ? `elite-shell-nav-${surface}` : undefined;
  const maxWidth =
    contentWidth === 'full' ? undefined : contentWidths[contentWidth];
  const renderShellTopbar =
    (surface === 'customer' || surface === 'admin') &&
    Boolean(nav) &&
    headerMode !== 'hidden';
  const shellPadding =
    headerMode === 'hidden'
      ...
      : renderShellTopbar
        ? 0
        : 'clamp(18px, 2vw, 26px)';
  const customerBreadcrumbLabel = breadcrumbLabel ?? title;
  const hasSubtitle = Boolean(subtitle.trim());
  const canToggleNav =
    Boolean(nav) && (surface === 'customer' || surface === 'admin');
  const resolvedSurfaceLabel = labels?.surfaceLabel ?? surfaceLabel(surface);
  const breadcrumbAriaLabel = labels?.breadcrumbAriaLabel ?? 'Breadcrumb';
  const breadcrumbHomeLabel = labels?.breadcrumbHomeLabel ?? 'Dashboard';
  const topbarAriaLabel =
    labels?.customerTopbarLabel ?? `${resolvedSurfaceLabel} topbar`;
  const mobileNavigationLabel = labels?.mobileNavigationLabel ?? 'Navigation';
  const mobileNavigationOpenLabel = labels?.mobileNavigationOpenLabel ?? 'Open';
  const collapseSidebarLabel =
    labels?.collapseSidebarLabel ?? 'Collapse sidebar menu';
  const expandSidebarLabel =
    labels?.expandSidebarLabel ?? 'Expand sidebar menu';
  const brandMarkAlt =
    labels?.brandMarkAlt ?? topbarBrandMarkAlt ?? 'Elite Message logo';
  ...
  return (
    ...
    >
      ...
      <div className="elite-shell-frame" data-has-nav={nav ? 'true' : 'false'}>
        {renderShellTopbar ? (
          <div className="elite-shell-topbar" aria-label={topbarAriaLabel}>
            <SurfaceBrand
              label={resolvedSurfaceLabel}
              variant="topbar"
              markSrc={
                surface === 'customer'
              ...
              markAlt={brandMarkAlt}
              launcherControlsId={navRegionId}
              launcherButtonLabel={collapseSidebarLabel}
              launcherExpandLabel={expandSidebarLabel}
              onLauncherClick={
                canToggleNav
                  ? (event) => {
                      const shell =
                      ...
                      const isCollapsed =
                        shell.getAttribute('data-nav-collapsed') === 'true';
                      setShellNavCollapsed(shell, !isCollapsed, navRegionId);
                    }
                  : undefined
              }
            />
            <div className="elite-shell-topbar-center">{meta}</div>
            <div className="elite-shell-topbar-actions">{headerActions}</div>
          </div>
        ) : null}
        {nav ? (
          <>
            <div className="elite-shell-mobile-nav">
              <details>
                <summary>
                  <span>{mobileNavigationLabel}</span>
                  <span aria-hidden="true">{mobileNavigationOpenLabel}</span>
                </summary>
                <div className="elite-shell-mobile-nav-body">{nav}</div>
              </details>
            </div>
            <aside id={navRegionId} className="elite-shell-aside">
              {nav}
            </aside>
          </>
        ) : null}

        <div className="elite-shell-main">
          <div
            className="elite-shell-content"
            style={{ maxWidth, margin: nav ? undefined : '0 auto' }}
          >
            {headerMode === 'hidden' ? null : (
              <header
                className={`elite-page-header${surface === 'customer' ? ' elite-page-header-customer' : ''}`}
              >
                {!renderShellTopbar ? (
                  <div className="elite-page-header-top">
                    <span className="elite-page-brand">
                      <span
                        className="elite-page-brand-mark"
                        aria-hidden="true"
                      />
                ...
                {surface === 'customer' ? (
                  <div className="elite-page-header-body elite-page-header-body-customer">
                    <div className="elite-page-header-row">
                      <h1 className="elite-page-title">{title}</h1>
                      <nav
                        className="elite-page-breadcrumbs"
                        aria-label={breadcrumbAriaLabel}
                      >
                        <a href="/">{breadcrumbHomeLabel}</a>
                        <span aria-hidden="true">/</span>
                        <span>{customerBreadcrumbLabel}</span>
                      </nav>
                    </div>
                    {hasSubtitle ? (
                      <p className="elite-page-subtitle">{subtitle}</p>
                    ) : null}
                  </div>
                ) : (
                  <div className="elite-page-header-body">
                    <h1 className="elite-page-title">{title}</h1>
                    {hasSubtitle ? (
                      <p className="elite-page-subtitle">{subtitle}</p>
                    ) : null}
            ...
            {secondaryNav ? (
              <div className="elite-page-secondary-nav">{secondaryNav}</div>
            ) : null}

            <div className="elite-page-stack">{children}</div>

            {footer ? (
              <footer className="elite-page-footer">{footer}</footer>
            ) : null}
          </div>
        </div>
      </div>
